...
function itemStatus(item) { if (pageView === 'payments') return item.paymentStatus || 'ยังไม่ชำระ'; return String(pageView === 'products' ? item.status : pageView === 'users' ? (item.role || item.friendStatus) : (item.paymentStatus || item.status) || 'ไม่ระบุ'); }
...
function renderControls(items) {
  const statuses = [...new Set(items.map(itemStatus).filter(Boolean))].slice(0, 10);
...
function renderList() {
  ...
  const visible = allItems.filter(item => (activeStatus === 'all' || itemStatus(item) === activeStatus) && (activeBranch === 'all' || item.branchName === activeBranch) && (!searchTerm || itemSearchText(item).includes(searchTerm)));
